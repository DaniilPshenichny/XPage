...
$dirtyPermissions = array_filter(json_decode($_POST['permissions']));
$permissions = [];
foreach ($dirtyPermissions as $id => $permission)$permissions[$id] = (array)$permission;
$iblockProvider = new \CIBlockElement(false);
$iblockProvider->Update($_POST['id'],[ 'NAME' => $_POST['roleTitle'], 'PROPERTY_VALUES' => ['112'=>json_decode($_POST['users'])]], true);
$permsDB = $iblockProvider->GetList([],[
    'IBLOCK_ID' => 31,
    'PROPERTY_111_VALUE' => $_POST['id']
],false,false,['ID','IBLOCK_ID','NAME','PROPERTY_112','IBLOCK_SECTION_ID']);
while($perm = $permsDB->Fetch()){
    $perms[] = $perm;
}
foreach ($perms as $permission){
    $resultPermissions[$permission['ID']] = [
        'read' => $permissions[$permission['IBLOCK_SECTION_ID']]['read'],
        'edit' => $permissions[$permission['IBLOCK_SECTION_ID']]['edit'],
        'vote' => $permissions[$permission['IBLOCK_SECTION_ID']]['vote'],
    ];
}
foreach ($resultPermissions as $key => $permission){
    $iblockProvider->Update($key,[
        'PROPERTY_VALUES' => [
            '108'=>$permission['read'],
            '109'=>$permission['edit'],
            '110'=>$permission['vote'],
   ...
   $userString = implode('|',$usersID);
    ...
    while($user = $usersDB->Fetch()){
        $users[$user['ID']] = mb_convert_encoding($user['LAST_NAME'] . ' ' . substr($user['NAME'],0,2) . '.' . substr($user['SECOND_NAME'],0,2) . '.' , "UTF-8");
    }
    foreach ($perms as $perm){
